...
def post(post_id):
    ...
    return render_template('post.html',
                           title=post_obj.title, post=post_obj, likes=res)
...
def like_action():
    if request.method == "POST":
        ...
        response = {}
        if like_obj:
            # like = Likes.query.filter(Likes.user_id == current_user.id).filter(Likes.post_id == post_id)\
            #     .filter(Likes.like == True).first()
